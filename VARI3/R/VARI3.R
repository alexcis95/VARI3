...
#' @import data.table
#' @import dplyr
#' @param bfile Es la ruta del archivo bfile de Plink.
#' @param out Es la ruta al directorio donde se guardará la salida de la función.
#' @param primarylist Es la rutal al archivo con los genes en los que deseamos que se realice la búsqueda de SNPs primarios. 
#' @param genelist Es la ruta al archivo con los genes en los que deseamos que se realice la búsqueda de epistasis o SNPs secundarios.
#' @param plink Es la ruta al ejecutable de plink.
#' @param core Por defecto selecciona todos los cores menos uno.
#' @param varpl Es la ruta al archivo annotate_variation.pl de ANNOVAR.
#' @param ANNOVAR Lógico, se recomienda que sea T. Anotación de las variantes con ANNOVAR.
#' @param clump Lógico, se recomienda que sea T si existen más de 2000 sujetos, realiza un clumping.
#' @param build Es la versión del genoma de referencia. 
#' @param db Es la ruta al directorio donde se encuentra la base de datos de ANNOVAR.
#' @param Wu Lógico, se recomienda que sea T. Realiza la corrección de Wu en epistasis.
#' @param covar Es la ruta al archivo de covariables.
#' @param ncov Es el índice de las covariables a introducir en el análisis.
#' @param AsT Por defecto 0.0001. Es el límite de p.valor en el que se seleccionan SNPs primarios. 
#' @param MAF Por defecto 0.01. Es el limite de MAF en el que se seleccionan SNPs primarios. 
#' @details Esta funcion es parte del doctorado de A.Cisterna para realizar un test de epistasis.
#' La documentacion de ayuda es generada usando roxygen2.
#' @export
    ...
    if (!is.null(covar)){
      ...
      if (clump){
        ...
        if (ANNOVAR){
          ...
          filtervariants <- filter(allassocano, allassocano$gen %like% paste(genes$GEN, collapse = "|") )
          
          names(filtervariants)[1] <- "CHR"
          filtervariants2 <- mutate(filtervariants, deseo = paste0("",CHR,":",SNP,""))
          
          write.table(c("SET", asclumanovar$SNPA, "END", "SET2", filtervariants2$deseo, "END"), file = paste0("",out,"/epistasisrset.set"), sep = " ",
    ...
    if (!is.null(covar)){
      ...
      if (clump){
        ...
        if (ANNOVAR){
          ...
          # filterprimary
          filterprimary <- filter(asclumanovar, asclumanovar$gen %like% paste(primary$GEN, collapse = "|") )
