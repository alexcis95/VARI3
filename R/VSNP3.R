...
VSNP3 = function(bfile,
      ...
      # Pensar en filtrar por frecuencia alelica si hay mucho
      assocrelevant = filter(assocrelevant, assocrelevant$F_A >= 0.03 | assocrelevant$F_U >= 0.03 )
      ...
      if (ANNOVAR){
          ...
          write.table(tx, file = paste0("",out,"/episelecrjoint.epi.cc"),
            ...
            # Generation of epistasis with WU joint effects correction (Same as CASSI, does not test for set so we have to use PLINK) Joint-effects applies correction that there are 5 per cell of the contingency table.
            system(paste0("",plink," --bfile ",bfile," --fast-epistasis joint-effects --set-test --set ",out,"/epistasisrset.set --out ",out,"/episelecrjoint --threads ",core," --allow-no-sex --epi1 0.0000001"))
