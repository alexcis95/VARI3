...
TLTO automates the conversion of the two locus ratios from plink to a graph with the odd ratios to better interpret the epistasis. To use the function it is necessary to have the following libraries installed: library(parallel), library(readr), library(data.table), library(dplyr), library(reshape2), library(tidyr) and library(ggplot2).  
...
PTMDR automates the selection of the best high order interaction SNPs with regard to a specific SNP selected and the conversion to MDR format. To use the function it is necessary to have the following libraries installed: library(parallel), library(readr), library(data.table), library(dplyr), library(reshape2), library(tidyr) and library(ggplot2).  
To use **PTMDR** it is necessary to have plink, epistasis file generated by VARI3 and the genotype of the cohort in bfile format.
To comment on the different variables that are configurable in **PTMDR**, let's detail their function and use:  
  
* **bfile**	The bfile variable is required, the user needs to define the path and name of the Plink bfile (bed, bim and fam) without the extension, the three files must be named in the same way. For example genotype.bed, genotype.bim, genotype.fam the variable bfile would be (bfile = "genotype"). 

* **epiall** The epiall variable is required, the user needs to define the path and name of the epistasis file generated by VARI3 usually episelecrjoint.epi.cc.  

* **snp**	The snp variable is required. It refers to the SNP that we want to find high order interactions and convert to MDR. 

* **plink** The plink variable is by default "plink" but it is necessary to say the path of the previously downloaded plink file if it has not been added to the system executable path.  

* **out** The out variable determines the output directory where all files generated by PTMDR will be written, ptmdr.txt will be the input file to MDR analysis. 

**For example:**  

~~~
PTMDR(bfile = "UNRELATED.SPAIN4.HARDCALLS.Rsq0.8", # bfile without extension
      epiall = "epiasocfilter/a1c1covs/episelecrjoint.epi.cc", # epistasis file generated by VARI3
      snp = "17:43472321",
      out = "Desktop/")
~~~
